...
if (MAINNET) {
  ...
  web3 = new Web3(
    new Web3.providers.HttpProvider(
      'https://mainnet.infura.io/v3/b2e13b0a648e4c67b4a36951f5b1ed62',
    ),
  )
} else {
  ...
  web3 = new Web3(
    new Web3.providers.HttpProvider(
      'https://ropsten.infura.io/v3/b2e13b0a648e4c67b4a36951f5b1ed62',
    ),
  )
}
...
async function getPrices(tokenFrom, tokenTo, quantityIn) {
  ...
  const ratesRequest = await fetch(
    `${API_ENDPOINT}/sell_rate?id=${address_1}&qty=1`,
  )
  const rates = await ratesRequest.json()
  const input_in_eth = rates.data[0].dst_qty

  const ratesRequest_2 = await fetch(
    `${API_ENDPOINT}/buy_rate?id=${address_2}&qty=1`,
  )
  // Parsing the output
...
export default async function executeSwap(tokenFrom, tokenTo, input_amount) {
  ...
  const ratesRequest_2 = await fetch(
    `${API_ENDPOINT}/buy_rate?id=${output_address}&qty=1`,
  )

  const rates_2 = await ratesRequest_2.json()
  console.log(
    '🚀 ~ file: useKyberSwap.js ~ line 104 ~ executeSwap ~ rates_2 =',
    rates_2,
  )
  const output_in_eth = rates_2.data[0].src_qty
...
async function Kyber_Token_for_ETH(token_address, QTY) {
  ...
  */
  ...
  const enabledStatusesRequest = await fetch(
  ...
  */
  ...
  const ratesRequest = await fetch(
    `${API_ENDPOINT}/sell_rate?id=${token_address}&qty=${QTY}`,
  )
  // Parsing the output
  const rates = await ratesRequest.json()
...
async function Kyber_ETH_for_Token(token_address, QTY, incrementNonce = false) {
  ...
  const ratesRequest = await fetch(
    `${API_ENDPOINT}/buy_rate?id=${token_address}&qty=${QTY}`,
  )
  ...
  */
  ...
  if (incrementNonce) {
    const transaction_count = await web3.eth.getTransactionCount(
      USER_ACCOUNT,
      'pending',
    )
