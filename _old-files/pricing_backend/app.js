...
async function Kyber(token_1, token_2) {
  ...
  let ratesRequest = await fetch('https://api.kyber.network/sell_rate?id=' + address_1 + '&qty=1');
  // Parsing the output
  let rates = await ratesRequest.json();
  // Getting the source quantity
  let input_in_eth = rates.data[0].dst_qty;

  let ratesRequest_2 = await fetch('https://api.kyber.network/buy_rate?id=' + address_2 + '&qty=1');
  // Parsing the output
